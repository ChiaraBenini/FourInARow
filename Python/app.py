...
def start_game(game_n: int, board: Board, players: List[PlayerController]) -> int:
    ...
    #some code to keep track of runtime and all counters
...
def get_players(game_n: int) -> List[PlayerController]:
    ...
    def choose_player(player_number: int, heuristic: Heuristic) -> PlayerController:
        print(f"\nChoose type for Player {player_number}:")
        print("1 - Human")
        print("2 - MinMax")
        print("3 - AlphaBeta ")
        print("4 - Monte Carlo ")
        ...
        while True:
            ...
            if choice == "1":
                return HumanPlayer(player_number, game_n, heuristic)
            elif choice == "2":
                while True:
                    try:
                        depth = int(input("Enter MinMax depth (e.g., 2 or 3): "))
                        return MinMaxPlayer(player_number, game_n, depth, heuristic)
                    except ValueError:
                        print("Please enter a valid integer for depth.")
            elif choice == "3":
                while True:
                    try:
                        depth = int(input("Enter AlphaBenta depth (e.g., 2 or 3): "))
                        return AlphaBetaPlayer(player_number, game_n, depth, heuristic)
                    except ValueError:
                        print("Please enter a valid integer for depth.")
            elif choice == "4":
                while True:
                    try:
                        num_playouts = int(input("Enter number of playouts you want to simulate: "))
                        return MonteCarloPlayer(player_number, game_n, num_playouts, heuristic)
                    except ValueError:
                        print("Please enter a valid integer for playouts.")
            else:
                print("Invalid choice. Please enter 1, 2, or 3.")

    human1: PlayerController = choose_player(1, heuristic1)
    human2: PlayerController = choose_player(2, heuristic2)
